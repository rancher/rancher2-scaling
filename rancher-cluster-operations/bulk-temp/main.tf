...
#   value = nonsensitive(rancher2_user.this[*])
#   sensitive = true
# }
...
# data "rancher2_user" "mine" {
#   name     = "project-bindings-users-bulk-user-0"
# }
...
resource "rancher2_global_role_binding" "this" {
  ...
  name           = "${local.global_role_binding_name_prefix}-${each.value.name}-${random_id.this[0].dec}"
...
#   name             = "${local.cluster_role_template_binding_name_prefix}-${each.value.name}-${random_id.this[0].dec}"
#   cluster_id       = data.rancher2_cluster.this.id
...
resource "rancher2_project_role_template_binding" "this" {
  ...
  name             = "${local.project_role_template_binding_name_prefix}-${each.value.name}-${random_id.this[0].dec}"
